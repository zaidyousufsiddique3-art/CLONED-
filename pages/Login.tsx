...
const InputField = ({ label, type = "text", value, onChange, placeholder, required = true, options, icon: Icon }: any) => (
    <div className="space-y-1.5 group">
        <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] ml-1 transition-colors group-focus-within:text-white">
            {label}
        </label>
        <div className="relative">
            {Icon && (
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 transition-colors group-focus-within:text-white">
                    <Icon size={16} />
                </div>
            )}
            {options ? (
                <div className="relative">
                    <select
                        value={value}
                        onChange={onChange}
                        className={`w-full ${Icon ? 'pl-11' : 'px-4'} py-3 bg-slate-900/50 border border-slate-800 rounded-xl focus:ring-1 focus:ring-white/20 focus:border-white/40 outline-none text-white appearance-none transition-all cursor-pointer hover:bg-slate-900/80 text-sm`}
                    >
                ...
                <input
                    ...
                    className={`w-full ${Icon ? 'pl-11' : 'px-4'} py-3 bg-slate-900/50 border border-slate-800 rounded-xl focus:ring-1 focus:ring-white/20 focus:border-white/40 outline-none text-white placeholder-slate-600 transition-all hover:bg-slate-900/80 text-sm`}
                />
            )}
        </div>
    </div>
...
const Login: React.FC = () => {
    ...
    return (
        <div className="h-screen bg-[#070708] flex flex-col font-sans selection:bg-slate-500/30 selection:text-white relative overflow-hidden">
            ...
            {/* Premium Background Effects: Subtle Gradients - Reduced Red */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                {/* Primary Subtle Glow - Darker/Cooler */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1200px] h-[800px] bg-slate-800/10 rounded-full blur-[140px]"></div>

                {/* Secondary Depth Accents - More Subtle */}
                <div className="absolute top-[-10%] right-[-5%] w-[700px] h-[700px] bg-slate-900/20 rounded-full blur-[120px]"></div>
                <div className="absolute bottom-[-10%] left-[-5%] w-[700px] h-[700px] bg-slate-900/20 rounded-full blur-[120px]"></div>
                ...
                <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-12 lg:gap-24 items-center mx-auto h-full max-h-[85vh]">
                        ...
                        <div className="space-y-8">
                            <div className="flex items-center gap-6 mb-4">
                                <div className="relative">
                                    ...
                                    <img src="/assets/logo.png" alt="SLISR Logo" className="relative h-32 w-auto object-contain drop-shadow-[0_0_30px_rgba(255,255,255,0.1)]" />
                                </div>
                            ...
                            <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-slate-800/50 border border-slate-700/50 text-slate-300 text-[10px] font-black tracking-[0.3em] uppercase shadow-lg backdrop-blur-sm">
                            ...
                            <h1 className="text-4xl lg:text-5xl font-black text-white tracking-tighter leading-[1.1]">
                                ...
                                <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-200 to-slate-400 whitespace-nowrap">Academic Documents</span>
                            </h1>
                    ...
                    <div className="flex justify-center lg:justify-start">
                        <div className="w-full max-w-md bg-[#0A0A0C]/80 backdrop-blur-3xl p-8 lg:p-12 rounded-[3rem] shadow-[0_30px_100px_rgba(0,0,0,0.9)] border border-white/5 relative overflow-hidden group">
                            ...
                            {/* Card Glow Effect - Very Subtle White/Blue */}
                            <div className="absolute -top-32 -right-32 w-64 h-64 bg-slate-800/10 rounded-full blur-[100px] group-hover:bg-slate-700/10 transition-colors duration-500"></div>
                            ...
                            {view === 'login' ? (
                                <div className="space-y-8 animate-fade-in relative z-10">
                                    ...
                                    {error && (
                                        <div className="bg-red-900/10 text-red-400 p-4 rounded-xl text-xs border border-red-900/20 font-bold flex items-center gap-3">
                                    ...
                                    <form onSubmit={handleLoginSubmit} className="space-y-6">
                                        ...
                                        <button type="submit" disabled={loading} className="w-full py-4 px-6 bg-white hover:bg-slate-200 disabled:bg-slate-700 text-black rounded-2xl font-black text-base tracking-wider shadow-lg shadow-white/10 hover:shadow-white/20 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed">
                                    ...
                                    <div className="space-y-6 pt-4">
                                        <div className="text-center space-y-3">
                                            <button onClick={() => setView('forgot')} className="text-sm text-slate-400 hover:text-white transition-colors font-semibold">
                                        ...
                                        <div className="pt-8 border-t border-slate-800/80 flex flex-col items-center gap-2.5 text-center">
                                            ...
                                            <Link to="/register" className="text-white font-black text-xs uppercase tracking-[0.2em] hover:text-slate-300 transition-colors group">
                                ...
                                <div className="space-y-8 animate-fade-in relative z-10">
                                    ...
                                    {resetSuccess ? (
                                        ...
                                        <form onSubmit={handleResetSubmit} className="space-y-4">
                                            <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-xl text-[10px] text-slate-400 leading-relaxed font-bold uppercase tracking-widest mb-4 text-center">
                                            ...
                                            <button
                                                type="submit"
                                                disabled={loading}
                                                className="w-full h-16 bg-white hover:bg-slate-200 text-black text-sm font-black rounded-2xl transition-all shadow-xl active:scale-[0.98] mt-4 uppercase tracking-[0.2em]"
                                            >
