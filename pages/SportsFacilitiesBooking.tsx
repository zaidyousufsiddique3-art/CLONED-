...
const SportsFacilitiesBooking: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'approved' | 'pending' | 'memberships'>('pending');
    ...
    useEffect(() => {
        ...
        // Fetch memberships
        const memQ = query(collection(db, 'badminton_memberships'), orderBy('purchaseDate', 'desc'));
    ...
    return (
        <div className="max-w-6xl mx-auto animate-fade-in pb-20">
            ...
            <div className="flex gap-4 mb-8 border-b border-white/10 overflow-x-auto whitespace-nowrap">
                <button
                ...
                <button
                    ...
                    className={`pb-4 px-4 text-sm font-bold border-b-2 transition-all flex items-center gap-2 ${activeTab === 'memberships' ? 'border-brand-600 text-brand-600' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                >
                    MEMBERSHIPS
                    <span className="bg-blue-100 text-blue-700 text-[10px] px-2 py-0.5 rounded-full">{memberships.length}</span>
                </button>
            </div>
            ...
            {activeTab !== 'memberships' ? (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                                {loading ? (
                                ...
                                ) : (activeTab === 'approved' ? approvedBookings : pendingAndHistoryBookings).length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="p-20 text-center">
                                    ...
                                    (activeTab === 'approved' ? approvedBookings : pendingAndHistoryBookings).map(booking => (
                                            ...
                                            <td className="p-6">
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {(booking as any).membership && (
                                                            <span className="text-[10px] font-black bg-emerald-500 text-white px-1.5 py-0.5 rounded uppercase tracking-tighter">
                                                                Paid via Membership
                                                            </span>
                                                        )}
                ...
                <div className="bg-[#070708] rounded-[2.5rem] p-8 border border-white/10 shadow-2xl animate-fade-in">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-white/5 bg-white/[0.02]">
                                    <th className="px-6 py-5 font-bold text-slate-500 uppercase tracking-widest text-[10px]">User</th>
                                    <th className="px-6 py-5 font-bold text-slate-500 uppercase tracking-widest text-[10px]">Type</th>
                                    <th className="px-6 py-5 font-bold text-slate-500 uppercase tracking-widest text-[10px]">Purchase Date</th>
                                    <th className="px-6 py-5 font-bold text-slate-500 uppercase tracking-widest text-[10px]">Expiry Date</th>
                                    <th className="px-6 py-5 font-bold text-slate-500 uppercase tracking-widest text-[10px]">Remaining Hours</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-white/5">
                                {memberships.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="px-6 py-20 text-center text-slate-500">No active memberships found.</td>
                                    </tr>
                                ) : memberships.map(m => (
                                    <tr key={m.id} className="hover:bg-white/[0.01] transition-colors group">
                                        <td className="px-6 py-5">
                                            <div className="text-sm font-bold text-white group-hover:text-brand-400 transition-colors uppercase">{m.userEmail}</div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <span className="text-[10px] font-black px-2 py-1 bg-brand-500/10 text-brand-500 rounded uppercase tracking-widest">{m.type}</span>
                                        </td>
                                        <td className="px-6 py-5 text-sm text-slate-400">{new Date(m.purchaseDate).toLocaleDateString()}</td>
                                        <td className="px-6 py-5 text-sm text-slate-400">{new Date(m.expiryDate).toLocaleDateString()}</td>
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-2">
                                                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                                                <span className="text-lg font-black text-emerald-500">{m.remainingHours} <span className="text-[10px]">HRS</span></span>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
